...
// Handle delete (same as admin view_events.php)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_event_id'])) {
  $delId = (int)$_POST['delete_event_id'];
  if ($delId > 0) {
    // Delete registrations first (if table exists) using prepared statement
    if ($stmtReg = $conn->prepare('DELETE FROM event_registrations WHERE event_id = ?')) {
      $stmtReg->bind_param('i', $delId);
      $stmtReg->execute();
      $stmtReg->close();
    }

    // Delete event via prepared statement
    if ($stmtEvt = $conn->prepare('DELETE FROM events WHERE event_id = ?')) {
      $stmtEvt->bind_param('i', $delId);
      if ($stmtEvt->execute()) {
...
// Fetch all events (exactly like admin)
if ($stmtList = $conn->prepare('SELECT event_id, event_name, category, subcategory, image_url FROM events ORDER BY category, subcategory, event_name')) {
  $stmtList->execute();
  $eventsRes = $stmtList->get_result();
  // We will not close $stmtList here before the HTML loop because mysqli_fetch_assoc is used on $eventsRes.
} else {
  $eventsRes = false;
}
?><!DOCTYPE html>
