...
// Ensure this coordinator is Sports category
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($approvalId > 0 && in_array($action, ['accept','reject'], true)) {
    // Load this approval row (must belong to this coordinator, Sports category)
    $stmtOne = $pdo->prepare('SELECT pa.*, p.status AS payment_status, p.amount FROM payment_approvals pa JOIN payments p ON pa.payment_id = p.id WHERE pa.id = ? AND pa.coord_id = ? AND pa.category = "Sports" LIMIT 1');
    $stmtOne->execute([$approvalId, $coordId]);
    $row = $stmtOne->fetch(PDO::FETCH_ASSOC);
    if ($row) {
      // Prevent Sports coordinators from acting on ₹350 Cultural-only payments
      if ((int)$row['amount'] === 350) {
        $errorMsg = '₹350 Cultural payments can only be approved by Cultural coordinators.';
      } else {
...
// Main Sports coordinator can see all Sports/Technical approvals; others see only their own approvals
$baseSql = 'SELECT pa.id AS approval_id, pa.status AS approval_status, pa.decided_at,
  ...
  p.payment_mode, p.utr_number, p.payment_date, p.decided_by_coord_id,
  p.requested_at, u.first_name, u.last_name, u.college,
  (SELECT COUNT(*) FROM payment_approvals WHERE payment_id = p.id AND status = "accepted") as approved_count,
  (SELECT COUNT(*) FROM payment_approvals WHERE payment_id = p.id AND category = "Sports") as total_sports_coords
FROM payment_approvals pa
JOIN payments p ON pa.payment_id = p.id
JOIN users u ON u.id = p.user_id
WHERE (pa.category = "Sports" OR pa.category = "Technical")';
...
<head>
  <meta charset="UTF-8">
  <title>Payment Requests (Sports)</title>
        ...
        <?php foreach ($rows as $r): ?>
          <tr>
            ...
            <td>
              ...
              // On Sports side, never allow actions on ₹350 payments
              $showActions = $isMine && !$isPaymentApproved && $r['approval_status'] === 'pending' && !$is350;
              ?>
