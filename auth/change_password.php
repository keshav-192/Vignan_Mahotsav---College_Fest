...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($current) || empty($new) || empty($confirm)) {
    // set specific field errors for empty values
  ...
  } else if (!password_verify($current, $hash)) {
    $ferr_current = "Current password is wrong.";
  } else if (!preg_match($password_regex, $new)) {
...
<head>
  ...
  <style>
    body {
      ...
      color: #eaf3fc; font-family:'Segoe UI',Arial,sans-serif;
    }
    .container {
    ...
    .pass-group { position: relative; margin-bottom: 24px; }
    ...
    .pass-group { margin-bottom: 4px; }
    .pass-group input { margin: 0; }
    button[type="submit"] {
    ...
    .container h2 { text-align: center; margin: 0 0 14px 0; }
  </style>
...
<body>
  ...
  <script>
    ...
    // Client-side inline validation similar to register page
    ...
    async function validateCurrent(){
      ...
      return new Promise(resolve => {
        currentCheckTimer = setTimeout(async () => {
          try {
            const resp = await fetch('verify_current_password.php', {
              method: 'POST',
    ...
    form.addEventListener('submit', async function(e){
      ...
      // On submit, if empty fields, show required messages immediately
